// Propiedad intelectual: Esta prueba fue desarrollada por TCIT el año 2014 y
// registrada bajo propiedad intelectual, cualquier publicación o difamación podría
// estar sujeta a acciones legales. Hay otras empresas que nos han copiado esta
// prueba, no aceptes imitaciones, exige el original xD
// No realizar la prueba en repl.it al hacerlo su respuesta queda disponible para
// otros postulantes, tampoco subirla a repositorios de github públicos
...
// parcela), el área está representada en m2, harvestYear es el año en el que se
// sembró el cuartel
const paddocks = [
  {
    paddockManagerId: 6,
...
on-line, la mejor forma de debuggear su código es usando un intérprete de
...
válidos.
 ...
 - Su prueba debe ejecutarse sin errores en la consola del inspector de Google
Chrome
*/
...
// 1 Arreglo con los ruts de los responsables de los cuarteles, ordenados por
// nombre;
function listPaddockManagersByName() {
  const paddocksManagersByName = [...paddockManagers].sort((a, b) =>
    a.name.localeCompare(b.name)
  );

  return paddocksManagersByName.map(
    (paddockManager) => paddockManager.taxNumber
  );
}
...
function sortPaddockTypeByTotalArea() {
  const totalAreaOfPaddocksByTypeId = {};

  for (const { paddockTypeId, area } of paddocks) {
    totalAreaOfPaddocksByTypeId[paddockTypeId] ??= 0;
    totalAreaOfPaddocksByTypeId[paddockTypeId] += area;
  }

  return paddockType
    .map((paddockTypeIndividual) => {
      return {
        ...paddockTypeIndividual,
        totalArea: totalAreaOfPaddocksByTypeId[paddockTypeIndividual.id],
      };
    })
    .sort(
      ({ totalArea: totalAreaA }, { totalArea: totalAreaB }) =>
        totalAreaB - totalAreaA
    )
    .map((paddock) => paddock.name);
}

// 3 Arreglo con los nombres de los administradores, ordenados decrecientemente por
// la suma TOTAL de hectáreas que administran.
function sortFarmManagerByAdminArea() {
  const totalAreaOfPaddocksByManagerId = {};
  for (const { paddockManagerId, area } of paddocks) {
    totalAreaOfPaddocksByManagerId[paddockManagerId] ??= 0;
    totalAreaOfPaddocksByManagerId[paddockManagerId] += area;
  }
  ...
  return paddockManagers
    .map((paddockManager) => {
      return {
        ...paddockManager,
        totalArea: totalAreaOfPaddocksByManagerId[paddockManager.id],
      };
    })
    .sort(
      ({ totalArea: totalAreaA }, { totalArea: totalAreaB }) =>
...
// arreglo con los ruts de sus administradores ordenados alfabéticamente por nombre.
function farmManagerNames() {
  return farms.reduce((result, farm) => {
    const grouppedPaddockManagersInFarm = paddocks.reduce((acc, paddock) => {
      if (paddock.farmId !== farm.id) return acc;

      acc[paddock.paddockManagerId] = true;
      return acc;
    }, {});

    result[farm.name] = paddockManagers
      .filter(
        (paddockManager) => grouppedPaddockManagersInFarm[paddockManager.id]
      )
...
function biggestAvocadoFarms() {
  ...
  farms.forEach((farm) => {
    const totalPaddocks = sortedPaddocksByArea.length;
    let totalArea = 0;
    ...
    for (const [
      index,
      { area: paddockArea, farmId: paddockFarmId, paddockTypeId },
    ] of sortedPaddocksByArea.entries()) {
      if (paddockFarmId !== farm.id) continue;

      totalArea += paddockArea;

      const paddockHasPaltosType = paddockTypeId === paltosTypeId;
      if (paddockHasPaltosType) {
        totalAreaOfPaltos += paddockArea;
      }

      remainingPaddocks = totalPaddocks - index - 1;
  ...
  return totalM2sByFarm.sort((farmM2sA, farmM2sB) => farmM2sB - farmM2sA);
}
...
// 9 Agregar nuevo administrador con datos ficticios a "paddockManagers" y agregar
// un nuevo cuartel de tipo NOGALES con 900mts2, año 2017 de AGRICOLA SANTA ANA,
